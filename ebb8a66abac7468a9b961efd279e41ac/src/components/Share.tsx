...
const Share = (props: any) => {
  ...
  const enterMeeting = () => {
    if (urlHost) {
      history.push(`/agora-call/${joinPhrase}`);
    }
  };
...
Meeting URL: ${$config.FRONTEND_ENDPOINT}/agora-call/${urlHost}`)
      ...
      ? hostControlCheckbox
        ? (stringToCopy += `Meeting - ${roomTitle}
URL for Attendee: ${window.location.origin}/agora-call/${urlView}
URL for Host: ${window.location.origin}/agora-call/${urlHost}`)
        : (stringToCopy += `Meeting - ${roomTitle}
Meeting URL: ${window.location.origin}/agora-call/${urlHost}`)
      : hostControlCheckbox
      ? (stringToCopy += `Meeting - ${roomTitle}
