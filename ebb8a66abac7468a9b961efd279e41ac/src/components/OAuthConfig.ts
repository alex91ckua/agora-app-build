...
const getRedirectFrontEndUrl = ({ platform}: {platform: platformType}) => platform === 'web' ? `${window.location.origin}`: `${window['AGORA_BACKEND_ENDPOINT']}`

export const oAuthGoogle = ({platform}: {platform: platformType}) => ({
  client_id: $config.GOOGLE_CLIENT_ID,
  auth_uri: 'https://accounts.google.com/o/oauth2/auth',
  redirect_uri: `${window['AGORA_BACKEND_ENDPOINT']}/oauth`,
  scope: encodeURIComponent(
    'https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/userinfo.email',
  ),
  state: encodeURIComponent(
    `site=google&platform=${platform}&backend=${window['AGORA_BACKEND_ENDPOINT']}&redirect=${getRedirectFrontEndUrl({platform})}/auth-token/`,
  ),
});
...
export const oAuthMicrosoft = ({platform}: {platform: platformType}) => ({
  ...
  scope: 'openid',
  state: encodeURIComponent(
    `site=microsoft&platform=${platform}&redirect=${getRedirectFrontEndUrl({platform})}/auth-token/&backend=${window['AGORA_BACKEND_ENDPOINT']}`,
  ),
});
...
export const oAuthSlack = ({platform}: {platform: platformType}) => ({
  ...
  state: encodeURIComponent(
    `site=slack&platform=${platform}&redirect=${getRedirectFrontEndUrl({platform})}/auth-token/&backend=${window['AGORA_BACKEND_ENDPOINT']}`,
  ),
});
...
export const oAuthApple = ({platform}: {platform: platformType}) => ({
  ...
  scope: 'name email',
  state: encodeURIComponent(
    `site=apple&platform=${platform}&redirect=${getRedirectFrontEndUrl({platform})}/auth-token/&backend=${window['AGORA_BACKEND_ENDPOINT']}`,
  ),
});
