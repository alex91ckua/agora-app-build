            ...
            <Router>
              <SessionProvider>
                <ColorConfigure>
                  <Navigation />
                  <Switch>
                    <Route exact path={'/agora-call/'}>
                      <Redirect to={'/agora-call/create'} />
                    </Route>
                    <Route exact path={'/agora-call/authenticate'}>
                      {shouldAuthenticate ? <OAuth /> : <Redirect to={'/'} />}
                    </Route>
                    <Route path={'/agora-call/auth-token/:token'}>
                      <StoreToken />
                    </Route>
                    <Route exact path={'/agora-call/join'}>
                      <Join phrase={phrase} onChangePhrase={onChangePhrase} />
                    </Route>
                    {shouldAuthenticate ? (
                      <PrivateRoute
                        path={'/agora-call/create'}
                        failureRedirectTo={'/agora-call/authenticate'}>
                        <Create />
                      </PrivateRoute>
                    ) : (
                      <Route path={'/agora-call/create'}>
                        <Create />
                      </Route>
                    )}
                    <Route path={'/agora-call/:phrase'}>
                      <VideoCall />
                    </Route>
                  </Switch>
                </ColorConfigure>
